...
def visualize_trajectory_predictions(raw_data_dict, save_path='./visualizations/', max_sequences=5):
    ...
    for step in list(raw_data_dict.keys())[:max_sequences]:
        ...
        for ped_idx in range(num_peds):
            # 观测轨迹（蓝色实线）
            line1 = plt.plot(obs_data[:, ped_idx, 0], obs_data[:, ped_idx, 1],
                             '-', color='blue', linewidth=2, alpha=0.8)
            if 'Observed' not in legend_added:
                legend_elements.append(mpatches.Patch(color='blue', label='Observed'))
                legend_added.add('Observed')

            # 真实轨迹（绿色实线）
            line2 = plt.plot(trgt_data[:, ped_idx, 0], trgt_data[:, ped_idx, 1],
                             '-', color='green', linewidth=2, alpha=0.8)
            if 'Ground Truth' not in legend_added:
                legend_elements.append(mpatches.Patch(color='green', label='Ground Truth'))
                legend_added.add('Ground Truth')

            # 预测轨迹（红色虚线，多个样本）
            for i, pred_sample in enumerate(pred_data[:3]):  # 只显示前3个预测样本
                line3 = plt.plot(pred_sample[:, ped_idx, 0], pred_sample[:, ped_idx, 1],
                                 '--', color='red', linewidth=1, alpha=0.6)
                if 'Predictions' not in legend_added:
                    legend_elements.append(mpatches.Patch(color='red', label='Predictions'))
                    legend_added.add('Predictions')
        ...
        for pred_idx, ax in enumerate(axes):
            ...
            for ped_idx in range(num_peds):
                ...
                if pred_idx < len(pred_data):
                    ...
                    if f'Prediction {pred_idx + 1}' not in legend_added:
                        legend_elements.append(mpatches.Patch(color='red', label=f'Prediction {pred_idx + 1}'))
                        legend_added.add(f'Prediction {pred_idx + 1}')
...
def visualize_uncertainty(raw_data_dict, save_path='./visualizations/', max_sequences=3):
    ...
    for step in list(raw_data_dict.keys())[:max_sequences]:
        ...
        for ped_idx in range(num_peds):
            ...
            if f'Ped {ped_idx} - Observed' not in legend_added:
                legend_elements.append(
                    plt.Line2D([0], [0], color='blue', linewidth=3, label=f'Ped {ped_idx} - Observed'))
                legend_added.add(f'Ped {ped_idx} - Observed')

            # 真实轨迹
            plt.plot(trgt_data[:, ped_idx, 0], trgt_data[:, ped_idx, 1],
                     '-', color='green', linewidth=3, alpha=0.9)
            if f'Ped {ped_idx} - Ground Truth' not in legend_added:
                legend_elements.append(
                    plt.Line2D([0], [0], color='green', linewidth=3, label=f'Ped {ped_idx} - Ground Truth'))
                legend_added.add(f'Ped {ped_idx} - Ground Truth')
            ...
            if 'Predictions (Uncertainty)' not in legend_added:
                legend_elements.append(
                    plt.Line2D([0], [0], color='red', linestyle='--', linewidth=1, label='Predictions (Uncertainty)'))
                legend_added.add('Predictions (Uncertainty)')
...
def visualize_prediction_error(raw_data_dict, save_path='./visualizations/', max_sequences=3):
    ...
    for step in list(raw_data_dict.keys())[:max_sequences]:
        ...
        for ped_idx in range(num_peds):
            ...
            if f'Ped {ped_idx} - Observed' not in legend_added:
                legend_elements.append(
                    plt.Line2D([0], [0], color='blue', linewidth=2, label=f'Ped {ped_idx} - Observed'))
                legend_added.add(f'Ped {ped_idx} - Observed')
            ...
            if f'Ped {ped_idx} - Ground Truth' not in legend_added:
                legend_elements.append(
                    plt.Line2D([0], [0], color='green', linewidth=2, label=f'Ped {ped_idx} - Ground Truth'))
                legend_added.add(f'Ped {ped_idx} - Ground Truth')

            # 预测轨迹
            plt.plot(pred_sample[:, ped_idx, 0], pred_sample[:, ped_idx, 1],
                     '--', color='red', linewidth=2, alpha=0.8)
            if f'Ped {ped_idx} - Prediction' not in legend_added:
                legend_elements.append(
                    plt.Line2D([0], [0], color='red', linestyle='--', linewidth=2, label=f'Ped {ped_idx} - Prediction'))
                legend_added.add(f'Ped {ped_idx} - Prediction')

            # 绘制误差向量（使用小箭头）
            for t in range(min(trgt_data.shape[0], pred_sample.shape[0])):
                if t % 3 == 0:  # 每隔3帧绘制一个箭头以避免过于密集
                    dx = pred_sample[t, ped_idx, 0] - trgt_data[t, ped_idx, 0]
                    dy = pred_sample[t, ped_idx, 1] - trgt_data[t, ped_idx, 1]

                    # 使用更小的箭头
                    plt.arrow(trgt_data[t, ped_idx, 0], trgt_data[t, ped_idx, 1],
                              dx, dy,
                              head_width=0.05, head_length=0.05,
                              fc='purple', ec='purple', alpha=0.5, width=0.01)
            if 'Prediction Errors' not in legend_added:
                legend_elements.append(
                    plt.Line2D([0], [0], color='purple', marker='>', linestyle='None', label='Prediction Errors'))
                legend_added.add('Prediction Errors')

        plt.xlabel('X coordinate')
        plt.ylabel('Y coordinate')
        plt.title(f'Prediction Error Visualization - Sequence {step}\n(Small purple arrows show prediction errors)')
...
def visualize_trajectory_density(raw_data_dict, save_path='./visualizations/', max_sequences=2):
    ...
    for step in list(raw_data_dict.keys())[:max_sequences]:
        ...
        # 创建2D直方图
        hist_plot = plt.hist2d(all_points[:, 0], all_points[:, 1], bins=50, cmap='Blues', alpha=0.7)
        ...
        for ped_idx in range(num_peds):
            ...
            if f'Ped {ped_idx} Observed' not in legend_added:
                legend_elements.append(plt.Line2D([0], [0], color=color, linewidth=2, label=f'Ped {ped_idx} Observed'))
                legend_added.add(f'Ped {ped_idx} Observed')
            ...
            if f'Ped {ped_idx} Ground Truth' not in legend_added:
                legend_elements.append(
                    plt.Line2D([0], [0], color=color, linestyle='--', linewidth=2, label=f'Ped {ped_idx} Ground Truth'))
                legend_added.add(f'Ped {ped_idx} Ground Truth')

            if pred_data:
                line3 = plt.plot(pred_data[0][:, ped_idx, 0], pred_data[0][:, ped_idx, 1],
                                 ':', color=color, linewidth=2, alpha=0.8)
                if f'Ped {ped_idx} Prediction' not in legend_added:
                    legend_elements.append(plt.Line2D([0], [0], color=color, linestyle=':', linewidth=2,
                                                      label=f'Ped {ped_idx} Prediction'))
                    legend_added.add(f'Ped {ped_idx} Prediction')
...
def generate_visualization_summary(raw_data_dict, ade_score, fde_score, save_path='./visualizations/'):
    ...
    if raw_data_dict:
        ...
        for ped_idx in range(num_peds):
            line1 = ax.plot(obs_data[:, ped_idx, 0], obs_data[:, ped_idx, 1],
                            '-', color='blue', linewidth=2, alpha=0.8)
            if 'Observed' not in legend_added:
                legend_elements.append(plt.Line2D([0], [0], color='blue', linewidth=2, label='Observed'))
                legend_added.add('Observed')

            line2 = ax.plot(trgt_data[:, ped_idx, 0], trgt_data[:, ped_idx, 1],
                            '-', color='green', linewidth=2, alpha=0.8)
            if 'Ground Truth' not in legend_added:
                legend_elements.append(plt.Line2D([0], [0], color='green', linewidth=2, label='Ground Truth'))
                legend_added.add('Ground Truth')

            if pred_data:
                line3 = ax.plot(pred_data[0][:, ped_idx, 0], pred_data[0][:, ped_idx, 1],
                                '--', color='red', linewidth=2, alpha=0.8)
                if 'Prediction' not in legend_added:
                    legend_elements.append(
                        plt.Line2D([0], [0], color='red', linestyle='--', linewidth=2, label='Prediction'))
                    legend_added.add('Prediction')
